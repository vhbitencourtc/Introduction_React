...
class PackageSourceLocal {
    ...
    async setupCacheDirectory() {
        ...
        if (config.dependencies) {
            ...
            const lockfile = await fs_1.promises.readFile(path_1.default.join(cacheDirectory, 'lock.json')).catch(() => null);
            if (lockfile) {
                await fs_1.promises.writeFile(path_1.default.join(packageSourceDirectory, 'package-lock.json'), lockfile);
            }
            else {
                await fs_1.promises.unlink(path_1.default.join(packageSourceDirectory, 'package-lock.json')).catch(() => null);
            }
        }
        else {
            await fs_1.promises.unlink(path_1.default.join(packageSourceDirectory, 'package.json')).catch(() => null);
            await fs_1.promises.unlink(path_1.default.join(packageSourceDirectory, 'package-lock.json')).catch(() => null);
        }
    }
    ...
    async buildPackageImport(spec, _source, logLine = false, depth = 0) {
        ...
        const newImportMap = await (async () => {
            ...
            const importMapHandle = await fs_1.promises.open(existingImportMapLoc, 'r+').catch(() => null);
